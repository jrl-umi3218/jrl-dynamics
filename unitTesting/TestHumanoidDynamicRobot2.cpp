...
//   };
  ...
  int lindex=0;
  for(int i=0;i<6;i++)    aCurrentConf[lindex++] = 0.0;
  ...
  cout << "Currentconf =  [";
  for(int i=0;i<aCurrentConf.size();++i )
    cout << aCurrentConf[i] << "  ";
  cout << " ]'; "<<endl;
  ...
  cout << "Jr = [ " << endl; DisplayMatrix(aJ); cout << " ];" <<endl ;

  {
    Joint  * aJointLeft = (Joint *)aVec[34]; // Try to get the hand.
    cout << "% "<< aJointLeft->getName() << endl;  
    aJointLeft->computeJacobianJointWrtConfig();
    
    MAL_MATRIX(,double) aJLeft = aJointLeft->jacobianJointWrtConfig();
    cout << "Jl = [ " << endl; DisplayMatrix(aJLeft); cout << " ];" <<endl ;
  }
  //exit(0);
  ...
  MAL_VECTOR_FILL(aCurrentAcc,10.0); //DEBUG: 10!
